...
| Endpoint                    | Change                                                                      |
| --------------------------- | --------------------------------------------------------------------------- |
| `GET /api/v1/events/live`   | Updated more frequently under streaming — source remains the `events` table |
| `GET /api/v1/alerts/recent` | Lower latency under streaming                                               |
| WebSocket `/ws/events`      | Receives real streaming data for the first time in M5                       |
